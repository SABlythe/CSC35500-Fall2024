...
TokenStream::TokenStream()
{
  ...
  // all transitions out of states 2 or higher are invalid
...
TokenStream::nextToken()
{
  ...
  if (state==-1)
    {
      ...
      //  behavior of such can be a bit unreliable on different systems. 
    }
  ...
  if (state!=-1) // if we cut out without seeing an invalid character ...
    ...
    t.type()=old_state; // otherwise old_state tells us the type. 
